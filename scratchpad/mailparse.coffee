...
onetimekey = ->
    BtsCrypto = require('../src/bts_crypto')
    aes = BtsCrypto.Aes.fromPassword('Password00')
    ...
    cipherhex = MailMessage.encryptedData message_data_hex
    ...
    setup_onetime_keys = ->
        ...
        ###
        onetime_public_key_compressed = MailMessage.oneTimeKey message_data_hex
    ...
    shared_secret_bitcore = ->
        ...
        # the private key
        S_privkey = S_kdf_buf.slice 32, 48
        S_iv = S_kdf_buf.slice 0, 32
        cipher_buf = new Buffer cipherhex, 'hex'
        ...
        symmetricDecrypt = (privkeyhex, ivhex, cipherhex) ->
            ...
            cipher = new sjcl.cipher.aes privbits
            
            # https://github.com/bitwiseshiftleft/sjcl/blob/master/core/cbc.js
            # https://github.com/bitwiseshiftleft/sjcl/issues/197
            decrypted = sjcl.mode.cbc.decrypt cipher, encbits, ivbits
            sjcl.codec.hex.fromBits decrypted
        plaintext = symmetricDecrypt S_privkey.toString('hex'), S_iv.toString('hex'), cipherhex
        ###
    ...
    elliptic = ->
        ...
        aes = BtsCrypto.Aes.fromSha512(shared_secret_sha512)
        b = ByteBuffer.fromHex(cipherhex)
        len = b.readVarint32()
        bcopy = b.copy(b.offset, b.offset + len)
        b.skip len
        assert.equal b.remaining(), 0, 'bytes unread '+b.remaining()
        plaintext = aes.decrypt_hex bcopy.toHex()
        b = ByteBuffer.fromHex(plaintext)
        b.skip 35
        plaintext = b.toHex()
